...
local list_org_en = {"County Hospital","City Hospital"}
...
--Departament
local dep = {
	list = {"["..u8:decode(PlayerSet.org()).."] - [Всем]", "[102,7 KHz] - Экстренная", "[104,8] - Для связи с МО/МЮ", "[109,6 kHz] - Для связи с Т.С.Р", "[103,9 kHz] - Собеседование", "[Информация] - Тех. неполадки","/gov - Новости"},
	sel_all = {u8"Всем", u8"Полиция Арзамаса", u8"Полиция Эдово", u8"Больница Арзамаса", u8"Больница Эдово", u8"Радиостанция Арзамаса", u8"Тюрьма", u8"Правительство", u8"Армия", u8"УФСБ", u8"Прокуратура", u8"Автошкола", u8"Министр Здравоохранения", u8"Министр Обороны", u8"Министр Юстиций"},
	sel_chp = {u8"Всем", u8"ГУ-МВД", u8"УГИБДД", u8"МЗ-А", u8"МЗ-Э", u8"РЦ", u8"ФСИН", u8"Право", u8"Армия", u8"УФСБ", u8"Прокуратура", u8"МРЭО", u8"МЗ", u8"МО", u8"МЮ"},
	--sel_tsr = {u8"Тюрьма", u8"Армия"},
	--sel_mzmomu = {u8"Полиция Арзамаса", u8"Полиция Эдово", u8"Тюрьма", u8"Армия", u8"УФСБ", u8"Автошкола", u8"Министр Здравоохранения", u8"Министр Обороны", u8"Министр Юстиций"},
	sel = imgui.ImInt(0),
	select_dep = {0, 0},
	input = imgui.ImBuffer(101),
...
cmdBind = {
	...
	[7] = {
		cmd = "/gt",
		key = {},
		desc = "Сокращённая команда /godeath",
		rank = 4,
		rb = false
	},
	[8] = {
	...
	[21] = {
		cmd = "/t",
		key = {},
		desc = "/time с отыгровкой(если включена)",
		rank = 5,
		rb = false
	}
}
...
--	repeat wait(100) until sampIsLocalPlayerSpawned()
	...
	------------
		...
		if not doesFileExist(dirml.."/MZHelper/files/logo-medicalhelper.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение logo-medicalhelper.png"); scr:unload() end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-logo.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-logo.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-site.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-site.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-role.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-role.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-med.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-med.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-nick.png") then print("{FF2525}Ошибка: {FFD825}Отсутствует изображение discord-nick.png") end
		...
		if not doesDirectoryExist(dirml.."/MZHelper/Шпаргалки/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для шпор")
			createDirectory(dirml.."/MZHelper/Шпаргалки/")
		end
		if not doesDirectoryExist(dirml.."/MZHelper/Департамент/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для новостей в департамент")
			createDirectory(dirml.."/MZHelper/Департамент/")
		end
		--Загрузка файла настроек
		if doesDirectoryExist(dirml.."/MZHelper/Департамент/") then
			getGovFile()
		end
...
{sleep:3000}
/me открыл{sex:|а} пустую мед.карту и паспорт, затем переписал{sex:|а} данные из паспорта
{sleep:3000}
...
Для оформления карты необходимо заплатить гос.пошлину, которая зависит от срока карты.
{sleep:3000}
На 7 дней #medup7 | 14 дней #medup14 | На 30 дней #medup30 | 60 дней #medup60
{sleep:3000} 
На сколько дней?
...
/medcard #playerID #healID #timeID]]  
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("t", funCMD.t)
		sampRegisterChatCommand("gt", funCMD.godeath)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Скрипт инициализирован.", 0xff00ff)

		repeat wait(100) until sampIsLocalPlayerSpawned()
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
		myNick = sampGetPlayerNickname(myid)

		sampAddChatMessage(string.format("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Приветствую, %s. Для активации главного меню пропишите в чат {22E9E3}/mz.", sampGetPlayerNickname(myid):gsub("_"," ")), 0xff00ff)
		wait(200)
		if buf_nick.v == "" then 
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Похоже у тебя не настроена основная информация. ", 0xff00ff)
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Зайди в главном меню в раздел \"Настройки\" и настрой себе всё по \"фэн-шую\".", 0xff00ff)
		end
...
function mainSet()
	...
	imgui.PushItemWidth(300);
		...
		imgui.PushItemWidth(278);
			...
			--		imgui.OpenPopup(u8"MZ | Изменение названия больницы")
			--	end
			--imgui.PopStyleVar(1)
			--imgui.SameLine(22)
			...
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##2", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"MZ | Изменение названия должности")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
	...
	imgui.PopItemWidth()
	imgui.EndGroup()
	if imgui.BeginPopupModal(u8"MZ | Изменение названия больницы", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	if imgui.BeginPopupModal(u8"MZ | Изменение названия должности", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	if mainWin.v then
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " MZHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			for i,v in ipairs(chgName.rank) do
				...
				f:flush()
				f:close()
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Настройки сохранены.", 0xff00ff)
				needSave = false
			end
			imgui.PopStyleColor(1)
			imgui.EndGroup()
						...
						if #spur.list ~= 20 then
							for i = 1, 20 do
								if not table.concat(spur.list, "|"):find("Шпаргалка '"..i.."'") then
									...
									spur.text.v = ""
									spurBig.v = false
									local f = io.open(dirml.."/MZHelper/Шпаргалки/Шпаргалка '"..i.."'.txt", "w")
									f:write("")
									f:flush()
									f:close()
									break
					...
					--	
						...
						--	imgui.SetCursorPosX(415)
							if imgui.Button(u8"Удалить", imgui.ImVec2(260, 20)) then
								if doesFileExist(dirml.."/MZHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
									os.remove(dirml.."/MZHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt")
								end
							...
							if imgui.Button(u8"Сохранить", imgui.ImVec2(260, 20)) then
								...
								if spur.name.v ~= "" then 
										...
										if doesFileExist(dirml.."/MZHelper/Шпаргалки/"..name..".txt") and spur.list[spur.select_spur] ~= name then
											bool = true
								...
								if not bool then
									local f = io.open(dirml.."/MZHelper/Шпаргалки/"..name..".txt", "w")
						...
						elseif not spurBig.v and (spur.select_spur >= 1 and spur.select_spur <= 20) then
							...
							if imgui.Button(u8"Редактировать", imgui.ImVec2(170, 20)) then
								spur.edit = true
								local f = io.open(dirml.."/MZHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt", "r")
				...
				imgui.EndChild();
					if cmdBind[selected_cmd].rank <= num_rank.v+1 then
						...
						if imgui.Button(u8"Назначить клавишу", imgui.ImVec2(140, 20)) then 
							imgui.OpenPopup(u8"MZ | Установка клавиши для активации");
			...
			if select_menu[6] then
				...
				imgui.BeginChild("help but", imgui.ImVec2(0,0), true)
					...
					if imgui.IsItemClicked(0) then
						print(shell32.ShellExecuteA(nil, 'open', dirml.."/MZHelper/Шпаргалки/", nil, nil, 1))
					end
					--
					imgui.Bullet(); imgui.SameLine()
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Spacing()
					imgui.TextWrapped(u8"\tСкрипт был разработан для проекта Rodina Role Play с поддержкой работы на сервере Западный Округ для облегчения работы сотрудникам больниц. Благодаря этому приложению Вы получите полный комплекс автоматизации многих действий и наслаждение от пользования.\nОбновления выходят по мере добавления нововведений и исправлений ошибок.")
					...
					--imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Нашли баг или ошибку, или же хотите видеть что-то новое, напиши нам"); imgui.SameLine(); imgui.Text(fa.ICON_ARROW_DOWN)
						imgui.SetCursorPosX(20)
						imgui.Text(fa.ICON_LINK)
						imgui.SameLine()
						imgui.TextColoredRGB("Для связи: VK: {74BAF4}https://vk.com/frostrylit") --или Discord - {74BAF4}TheVitek#2160")
						...
						if imgui.Button(u8"Удалить скрипт", imgui.ImVec2(160, 20)) then 
							...
							sampAddChatMessage("", 0xff00ff)
							sampAddChatMessage("", 0xff00ff)
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Внимание! Подтвердите удаление командой {77DF63}/mh-delete.", 0xff00ff)
							mainWin.v = false
						--	sampShowDialog(1002, "{E94C4C}MZHelper | {8EE162}Удаление", remove, "Принял", "")
						end
				imgui.EndChild()
			...
			--///Установка клавиши
				...
				if imgui.BeginPopupModal(u8"MZ | Установка клавиши для активации", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	--	imgui.SetWindowFontScale(1.1)
		if spur.edit then
				...
				if imgui.Button(u8"Удалить", imgui.ImVec2(357, 20)) then
					...
					table.remove(spur.list, spur.select_spur) 
					spur.select_spur = -1
					if doesFileExist(dirml.."/MZHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt") then
						os.remove(dirml.."/MZHelper/Шпаргалки/"..u8:decode(spur.select_spur)..".txt")
					end
					spur.name.v = ""
					spurBig.v = false
					spur.edit = false
			...
			imgui.BeginChild("spur spec", imgui.ImVec2(1081, 730), true)
				if doesFileExist(dirml.."/MZHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") then
					for line in io.lines(dirml.."/MZHelper/Шпаргалки/"..spur.list[spur.select_spur]..".txt") do
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8"Начать", imgui.ImVec2(60, 20)) then
					if sobes.selID.v ~= "" then
						...
						sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Укажите id игрока для начала собеседования.", 0xff00ff)
					end
				end
		...
		--imgui.SetWindowFontScale(1.1)
			imgui.BeginGroup()
				...
				imgui.EndChild()
					...
					--	imgui.Dummy(imgui.ImVec2(0, 65)) 
					--	if imgui.Button(u8"Объявить", imgui.ImVec2(225, 30)) then 
					--		sampSendChat(string.format("/d [%s] - [Всем] Рация упала.", u8:decode(PlayerSet.org()))) 
					--	end
					elseif dep.bool[7] then
						...
						imgui.Spacing()
							...
							if imgui.IsItemClicked(0) then
								print(shell32.ShellExecuteA(nil, 'open', dirml.."/MZHelper/Департамент/", nil, nil, 1))
							end
						imgui.Spacing()
						imgui.Spacing()
						if imgui.Button(u8"Подать", imgui.ImVec2(225, 30)) then
							...
							-- 	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Gov Новости назначить можно только от 0 до 15 минут.", 0xff00ff)
							-- 	return
							-- end
							
								if doesFileExist(dirml.."/MZHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") then
								--	print(u8:decode(dep.news[dep.newsN.v+1]))
									for line in io.lines(dirml.."/MZHelper/Департамент/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") do
										sampSendChat(line)
										wait(3000)
									end
								end
					...
					elseif dep.select_dep[2] < 5 and dep.select_dep[2] ~= 0 then
						...
						imgui.TextDisabled(u8"End - Открыть чат с шаблоном")
							if wasKeyReleased(VK_END) and not sampIsChatInputActive() then
								sampSetChatInputText(string.format("/d [%s] - [%s]: ", rankFix(), u8:decode(dep.sel_chp[dep.sel.v+1])));
						...
						--	imgui.TextDisabled(u8"End - Открыть чат с шаблоном")
						--	if wasKeyReleased(VK_END) and not sampIsChatInputActive() then
						--		sampSetChatInputText(string.format("/d [%s] - [%s]: ", rankFix(), u8:decode(dep.sel_mzmomu[dep.sel.v+1])));
						--		sampSetChatInputEnabled(true) 
						--	end
						...
						--imgui.TextDisabled(u8"End - Открыть чат с шаблоном")
						--	if wasKeyReleased(VK_END) and not sampIsChatInputActive() then
						--		sampSetChatInputText(string.format("/d [%s] - [%s]: ", rankFix(), u8:decode(dep.sel_tsr[dep.sel.v+1])));
						--		sampSetChatInputEnabled(true) 
						--	end
						end
						imgui.PopItemWidth()
			...
			imgui.BeginChild("dep log", imgui.ImVec2(0, 0), true)
				...
				if imgui.Button(u8"Отправить", imgui.ImVec2(80, 20)) then
					if dep.select_dep[2] < 5 and dep.select_dep[2] > 0 then
						if dep.bool[1] then
							sampSendChat(string.format("/d [%s] - [%s]: "..u8:decode(dep.input.v), rankFix(), u8:decode(dep.sel_chp[dep.sel.v+1])))
						--elseif dep.bool[2] then
						--	sampSendChat(string.format("/d [%s] - [%s]: "..u8:decode(dep.input.v), rankFix(), u8:decode(dep.sel_chp[dep.sel.v+1])))
						--elseif dep.bool[3] then
						--	sampSendChat(string.format("/d [%s] - [%s]: "..u8:decode(dep.input.v), rankFix(), u8:decode(dep.sel_chp[dep.sel.v+1])))
						...
						if removeColorTags(dText:match("Имя:%s*(%S+%s*%S+)")) == sobes.player.name then
							...
							if sobes.player.let >= 3 then
								...
								table.insert(sobes.logChat, "{54A8F2}"..sobes.player.name.."{FFFFFF}: Показал(а) паспорт. Мало проживает в штате.")
									wait(3000)
									sampSendChat("Извиняюсь, но Вы нам не подходите.Необходимо как минимум проживать 3 года в штате.")
									wait(3000)
...
function r(str)
    if buf_teg.v ~= "" and #str >= 1 then
        sampSendChat(string.format("/r [%s] %s", u8:decode(buf_teg.v), str))
	else
	...
	if thread:status() == "dead" then
		...
		for k, v in pairs(cmdBind) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				...
				elseif k == 8 then
					if resTarg then
						funCMD.godeath(tostring(targID))
					else
						sampSetChatInputEnabled(true)
						sampSetChatInputText("/godeath ")
					end
				elseif k == 9 then
					if resTarg then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: В данный момент проигрывается отыгровка.", 0xff00ff)
	end
end
...
local function playBind(tb)
	if not tb.debug.file or #tb.debug.close > 0 then
		if not tb.debug.file then
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Файл с текстом бинда не обнаружен. ", 0xff00ff)
...
function getSpurFile()
	spur.list = {}
    local search, name = findFirstFile("moonloader/MZHelper/Шпаргалки/*.txt")
	while search do
...
function getGovFile()
local mzA = [[
/d [МЗ-А] - [Всем]: Занял государственную волну новостей
/gov [Городская больница] Уважаемые жители, минуточку внимания.
/gov [Городская больница] Сейчас пройдет собеседование в Городскую больницу.
/gov [Городская больница] Критерии для вступления : Паспорт, лицензии, мед.карта, проживание от 3х лет.
/d [МЗ-А] - [Всем]: Освобождаю государственную волну новостей
]]
local mzE = [[
/d [МЗ-Э] - [Всем]: Занял государственную волну новостей
/gov [Больница округа] Уважаемые жители, минуточку внимания.
/gov [Больница округа] Сейчас пройдет собеседование в Больницу округа.
/gov [Больница округа] Критерии для вступления : Паспорт, лицензии, мед.карта, проживание от 3х лет.
/d [МЗ-Э] - [Всем]: Освобождаю государственную волну новостей
]]
--local govlv = [[
...
lua_thread.create(function()
	if doesDirectoryExist(dirml.."/MZHelper/Департамент/") then
		if doesFileExist(dirml.."/MZHelper/Департамент/Собеседование.txt") or not doesFileExist(dirml.."/MZHelper/Департамент/Собеседование.txt") then
			os.remove(dirml.."/MZHelper/Департамент/Собеседование.txt")
			local f = io.open(dirml.."/MZHelper/Департамент/Собеседование МЗ-А.txt", "w")
			f:write(mzA)
			f:flush()
			f:close()
			local f = io.open(dirml.."/MZHelper/Департамент/Собеседование МЗ-Э.txt", "w")
			f:write(mzE)
			f:flush()
			f:close()
			--local f = io.open(dirml.."/MZHelper/Департамент/День открытых дверей ЛВМЦ.txt", "w")
			--f:write(govlv)
			--f:flush()
			--f:close()
		end
		dep.news = {}
		local search, name = findFirstFile("moonloader/MZHelper/Департамент/*.txt")
		while search do
			if not name then findClose(search) else
...
function tags(par)
		...
		if par:find("{getNickByID:%d+}") then
			for v in par:gmatch("{getNickByID:%d+}") do
				...
				if sampIsPlayerConnected(id) then
					...
					sampAddChatMessage("{FFFFFF}[{ff00ff}MZ:Ошибка{FFFFFF}]: Параметр {getNickByID:ID} не смог вернуть ник игрока. Возможно игрок не в сети.", 0xff00ff)
					par = par:gsub(v,"")
				end
			end
		end
		...
		if par:find("{getNickByTarget}") then
			if targID ~= nil and targID >= 0 and targID <= 1000 and sampIsPlayerConnected(targID) then
				...
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZ:Ошибка{FFFFFF}]: Параметр {getNickByTarget} не смог вернуть ник игрока. Возможно Вы не целились на игрока, либо он не в сети.", 0xff00ff)
...
funCMD = {} 
function funCMD.del()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Вы успешно удалили скрипт.", 0xff00ff)
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Выгрузка скрипта из игры...", 0xff00ff)
	os.remove(scr.path)
	scr:reload()
end
function funCMD.lec(id)
	...
	if not u8:decode(buf_nick.v):find("[а-яА-Я]+%s[а-яА-Я]+") then
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Подождите-ка, сначала нужно заполнить базовую информацию. {90E04E}/mz > Настройки > Основная информация", 0xff00ff)
		return
	end
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				sampSendChat(string.format("Здравствуйте, я %s, дежурный лечащий врач.", u8:decode(buf_nick.v)))
				wait(3000)
					--sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Нажмите на  {23E64A}Enter{FFFFFF} для продолжения.", 0xff00ff)
					--local audio = loadAudioStream('moonloader/MZHelper/music/up.mp3')
					--setAudioStreamState(audio, 1)
					...
					--	if isKeyJustPressed(VK_RETURN) and not sampIsChatInputActive() and not sampIsDialogActive() then break end
					--end
				--sampSendChat("/do Медицинская сумка на левом плече")
				--wait(2000)
				sampSendChat(chsex("/me открыв сумку, достал из неё нужную таблетоку", "/me открыв сумку, достала из неё нужную таблетоку"))
				wait(3000)
				--sampSendChat(chsex("/me выдавил таблетку из пачки", "/me выдавила таблетку из пачки"))
				--wait(2000)
				sampSendChat(chsex("/me передал таблетку больному", "/me передала таблетку больному"))
				wait(3000)
				--sampSendChat("/do Лекарство в левой руке.")
				--wait(2000)
				sampSendChat("Хорошего дня, выздоравливайте")
				wait(100)
				sampSendChat("/heal "..id.." 1000")
	...
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /hl [id игрока].", 0xff00ff)
	end
end
function funCMD.med(id)
	...
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду: /mc [id игрока].", 0xff00ff)
	end
...
--			end)
--		else
--		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /narko [id игрока].", 0xff00ff)
--		end
--end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(3000)
				sampSendChat("Хорошо, стоимость одного рецепта 5.000")
				wait(3000)
				--sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели равная количеству выдаваемых рецептов.", 0xff00ff)
				...
				wait(3000)
				sampSendChat("/me "..chsex("заполнил","заполнила").." бланки на оформление лекарств")
				wait(3000)
				sampSendChat("/me поставил".. chsex("", "а") .." печать "..u8:decode(chgName.org[num_org.v+1]))
				wait(3000)
				sampSendChat("Вот Ваш рецепт, всего доброго.")
				wait(1000)
				sampSendChat("/recept "..id.." 5000")
				countRec = 1
			end)
		else
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /rec [id игрока].", 0xff00ff)
		end
end
...
--			else
--				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Укажите обстановку, например, /post Спокойно.", 0xff00ff)
--			end
--		end
--	end
--end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(3000)
				sampSendChat("Стоимость выведения татуировки составит 10.000, Вы согласны?")
				wait(3000)
				--sampSendChat("/n Оплачивать не требуется, сервер сам предложит")
				--wait(2000)
				...
				wait(150)
				sampSendChat("/unstuff "..id.." 10000")
				wait(5000)
				sampSendChat("Всё, ваш сеанс закончен. Всего Вам хорошего!?")
			end)
		else
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /tatu [id игрока].", 0xff00ff)
		end	
end
function funCMD.warn(text)
	...
	if num_rank.v+1 < 8 then
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Данная команда Вам недоступна. Поменяйте должность в настройках скрипта, если это требуется.", 0xff00ff)
		return
	end
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /+warn [id игрока] [причина].", 0xff00ff)
		end
end
function funCMD.uwarn(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /-warn [id игрока].", 0xff00ff)
		end
end
function funCMD.inv(id)
	...
	if num_rank.v+1 < 9 then
		...
		if id:find("(%d+)") then
		thread = lua_thread.create(function()
					sampSendChat("/me "..chsex("передал","передала").." ключ от шкафчика №"..id.." с формой Санитара человеку напротив.")
					wait(3000)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /inv [id игрока].", 0xff00ff)
		end
end
function funCMD.unv(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /unv [id игрока] [причина].", 0xff00ff)
		end
end
function funCMD.mute(text)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /+mute [id игрока] [время в минутах] [причина].", 0xff00ff)
		end
end
function funCMD.umute(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /-mute [id игрока].", 0xff00ff)
		end
end
function funCMD.rank(text)
	...
	if num_rank.v+1 < 9 then
		...
		thread = lua_thread.create(function()
					...
					wait(150)
					sampSendChat("/r У сотрудника с бейджиком №"..id.." новая должность.")
			end)
		else
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /gr [id игрока] [номер ранга].", 0xff00ff)
		end
end
function funCMD.osm()
	if thread:status() ~= "dead" then 
...
function funCMD.godeath(id)
	if id then
		sampSendChat("/godeath "..id)
	else
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду: /gt [id игрока].", 0xff00ff)
	end
end
...
--		end)
--	else
--	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Используйте команду /exp [id игрока] [Причина].", 0xff00ff)
--	end
--end
		
function funCMD.openupd()
	print(shell32.ShellExecuteA(nil, 'open', " ", nil, nil, 1))
end

function funCMD.update()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Производится скачивание новой версии скрипта...", 0xff00ff)
	local dir = dirml.."/MZHelper.lua"
	local url = "https://https://github.com/JamesHarriso/MzHelper/blob/main/MZHelper.lua?raw=true"
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		...
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Скачивание завершено, перезагрузка библиотек...", 0xff00ff)
			reloadScripts()
			showCursor(false)
		end
	end)
...
Либо скачайте файл скрипта отдельно. Ссылка для скачивания уже скопирована.
	...
	3. Удалите файл MZHelper.lua
	4. Переместите скаченный файл в папку Moonloader. 
	{FCB32B}5. Убедитесь, что название не содержит лишних символов, например MZHelper{F65050}(1){FCB32B}.luac
]]
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Производится скачивание новой версии скрипта...", 0xff00ff)
	local dir = dirml.."/MZHelper.lua"
	local url = urlupd
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				setClipboardText("https://raw.githubusercontent.com/JamesHarriso/MzHelper/main/MZHelper.lua?raw=true")
				updWin.v = false
			end
		end
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
...
function funCMD.updateCheck()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Проверяем наличие обновлений...", 0xff00ff)
		local dir = dirml.."/MZHelper/files/update.med"
		local url = "https://raw.githubusercontent.com/JamesHarriso/MzHelper/main/update.med"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
				wait(1000)
				if doesFileExist(dirml.."/MZHelper/files/update.med") then
					...
					urlupd = upd.url
						if upd.version == scr.version then
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Всё отлично, Вы используете самую новую версию скрипта.", 0xff00ff)
						else
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: {4EEB40}Имеется обновление.{FFFFFF}", 0xff00ff) --Напиши {22E9E3}/update{FFFFFF} для получения информации.
							wait(5000)
							updWin.v = true
						end
					end
		...
		local dir = dirml.."/MZHelper/files/update.txt"
		local url = "https://raw.githubusercontent.com/JamesHarriso/MzHelper/main/update.txt"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
					wait(1000)
...
function hook.onServerMessage(mesColor, mes) -- HOOK
	...
	if mes:find("%[D%] [%X%a]+ [%a_]+%[%d+%]: %[[%X%a]+%].+%[МЗ%]") and num_rank.v == 10 then
		local org = mes:match("%[D%] [%X%a]+ [%a_]+%[%d+%]: %[([%X%a]+)%].+%[МЗ%]")
		if mes:find("связь") and num_rank.v > 3 then -- rankFix()
			addOneOffSound(0, 0, 0, 1085)
			addOneOffSound(0, 0, 0, 1085)
			table.insert(dep.dlog, "{40ABF7}[D] {7ECAFF}["..org.."]: {FFFFFF}Вызывает на связь!")
		...
		if depWin.v and dep.select_dep[2] < 5 and dep.select_dep[2] > 0 then
			local mesD = mes:match("%[D%] [%X%a]+ [%a_]+%[%d+%]: %[[%X%a]+%].+%[МЗ%]%p*(.+)")
			table.insert(dep.dlog, "{40ABF7}[D] {7ECAFF}["..org.."]: {FFFFFF}"..mesD)
		end
	elseif mes:find("%[D%] [%X%a]+ [%a_]+%[%d+%]: %[МЗ%].+%[[%X%a]+%]") and num_rank.v == 10 then
		if depWin.v and dep.select_dep[2] < 5 and dep.select_dep[2] > 0 then
			local mesD = mes:match("%[D%] [%X%a]+ [%a_]+%[%d+%]: %[МЗ%].+%[[%X%a]+%]%p*(.+)")
			table.insert(dep.dlog, "{40ABF7}[D] {F55C5C}[МЗ]: {FFFFFF}"..mesD)
		end
	end
end
...
function hook.onSendCommand(cmd)
	if cmd:find("/r ") then
		if cb_rac.v then
			lua_thread.create(function()
			wait(700)
			sampSendChat(u8:decode(buf_teg.v))
			end)
		end
	end
end
