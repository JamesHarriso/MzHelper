...
if not doesFileExist(getGameDirectory().."/SAMPFUNCS.asi") then
	...
	ffi.C.MessageBoxA(0, sampfuncsNot, "MZHelper", 0x00000030 + 0x00010000) 
end
if #nofiles > 0 then
	ffi.C.ShowWindow(ffi.C.GetActiveWindow(), 6)
	ffi.C.MessageBoxA(0, errorText:format(table.concat(nofiles, "\n\t\t")), "MZHelper", 0x00000030 + 0x00010000) 
end
...
chgName.org = {u8"ÌÇ-À", u8"ÌÇ-Ý"}
chgName.rank = {u8"Ñàíèòàð ", u8"Ôåëüäøåð", u8"Âðà÷-ïðàêòèêàíò", u8"", u8"Âðà÷-ñïåöèàëèñò", u8"Òåðàïåâò", u8"Íàðêîëîã", u8"Õèðóðã", u8"Çàâåäóþùèé îòäåëåíèåì", u8"Çàì. Ãëàâíîãî âðà÷à", u8"Ãëàâíûé âðà÷"}

local list_org_BL = {"ÌÇ-À", "ÌÇ-Ý"} 
local list_org	= {u8"ÌÇ-À", u8"ÌÇ-Ý"}
...
local list_rank	= {u8"Ñàíèòàð ", u8"Ôåëüäøåð", u8"Âðà÷-ïðàêòèêàíò", u8"", u8"Âðà÷-ñïåöèàëèñò", u8"Òåðàïåâò", u8"Íàðêîëîã", u8"Õèðóðã", u8"Çàâåäóþùèé îòäåëåíèåì", u8"Çàì. Ãëàâíîãî âðà÷à", u8"Ãëàâíûé âðà÷"}
--chat
...
function imgui.BeforeDrawFrame()
  if fa_font == nil then
    ...
    fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/MZHelper/files/font-icon.ttf', 15.0, font_config, fa_glyph_ranges)
  end
end
...
function main()
	...
	if script.this.filename:find("%.luac") then
		os.rename(getWorkingDirectory().."\\MZHelper.luac", getWorkingDirectory().."\\MZHelper.lua") 
	end
--	repeat wait(100) until sampIsLocalPlayerSpawned()
	------------
	...
	------------
		...
		if not doesFileExist(dirml.."/MZHelper/files/logo-medicalhelper.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå logo-medicalhelper.png"); scr:unload() end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-logo.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå discord-logo.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-site.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå discord-site.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-role.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå discord-role.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-med.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå discord-med.png") end
		if not  doesFileExist(dirml.."/MZHelper/files/discord-nick.png") then print("{FF2525}Îøèáêà: {FFD825}Îòñóòñòâóåò èçîáðàæåíèå discord-nick.png") end
		logoMH = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/logo-medicalhelper.png") 
		logoDis = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/discord-logo.png")
		
		--Ïðîâåðêà íà ñóùåñòâîâàíèå ïàïêîê
		if not doesDirectoryExist(dirml.."/MZHelper/files/") then
			print("{F54A4A}Îøèáêà. Îòñóòñòâóåò ïàïêà. {82E28C}Ñîçäàíèå ïàïêè ïîä ôàéëû")
			createDirectory(dirml.."/MZHelper/files/")
		end
		if not doesDirectoryExist(dirml.."/MZHelper/Binder/") then
			print("{F54A4A}Îøèáêà. Îòñóòñòâóåò ïàïêà. {82E28C}Ñîçäàíèå ïàïêè äëÿ áèíäåðà.")
			createDirectory(dirml.."/MZHelper/Binder/")
		end
		if not doesDirectoryExist(dirml.."/MZHelper/Øïàðãàëêè/") then
			print("{F54A4A}Îøèáêà. Îòñóòñòâóåò ïàïêà. {82E28C}Ñîçäàíèå ïàïêè äëÿ øïîð")
			createDirectory(dirml.."/MZHelper/Øïàðãàëêè/")
		end
		if not doesDirectoryExist(dirml.."/MZHelper/Äåïàðòàìåíò/") then
			print("{F54A4A}Îøèáêà. Îòñóòñòâóåò ïàïêà. {82E28C}Ñîçäàíèå ïàïêè äëÿ íîâîñòåé â äåïàðòàìåíò")
			createDirectory(dirml.."/MZHelper/Äåïàðòàìåíò/")
		end
		--Çàãðóçêà ôàéëà íàñòðîåê
		if doesDirectoryExist(dirml.."/MZHelper/Äåïàðòàìåíò/") then
			getGovFile()
		end
		if doesFileExist(dirml.."/MZHelper/rp-medcard.txt") then
			local f = io.open(dirml.."/MZHelper/rp-medcard.txt")
			buf_mcedit.v =  u8(f:read("*a"))
			f:close()
			print("{82E28C}×òåíèå îòûãðîâêè ìåä.êàðòû...")
		else 
			local textrp = [[
// Öåíû íà âûäà÷ó íîâîé ìåä.êàðòû
#med7=10.000$
#med14=20.000$
#med30=32.500$
#med60=65.000$
// Öåíû íà îáíîâëåíèå ìåä.êàðòû
#medup7=15.000$
#medup14=25.000$
#medup30=38.000$
#medup60=70.000$
...
{sleep:0}
Çäðàâñòâóéòå, Âû õîòèòå ïîëó÷èòü ìåäèöèíñêóþ êàðòó âïåðâûå èëè îáíîâèòü ñóùåñòâóþùóþ?
Ïðåäîñòàâüòå, ïîæàëóéñòà, Âàø ïàñïîðò
/b /showpass {myID}
{pause}
...
[2]=Îáíîâëåíèå äàííûõ
Õîðîøî, ÿ Âàñ ïîíÿë{sex:|à}. Âàì íóæíî îáíîâèòü äàííûå â ìåä.êàðòå.
Äëÿ îáíîâëåíèÿ äàííûõ íåîáõîäèìî çàïëàòèòü ãîñ.ïîøëèíó, êîòîðàÿ çàâèñèò îò ñðîêà êàðòû.
Íà 7 äíåé - #medup7, íà 14 äíåé - #medup14
Íà 30 äíåé #medup30, íà 60 äíåé - #medup60.
Âû ñîãëàñíû?
Åñëè ñîãëàñíû, òî îïëàòèòå è ìû ïðîäîëæèì ïðîöåññ îôîðìëåíèÿ.
/b Îïëàòèòü ìîæíî ÷åðåç /pay {myID} èëè /trade {myID}
...
Æàëîáû íà çäîðîâüå èìåþòñÿ?
{pause}
Èìåþòñÿ ëè âðåäíûå ïðèâû÷êè, à òàêæå àëëåðãè÷åñêèå ðåàêöèè?
{pause}
...
/medcard #playerID #healID #timeID 1000]]  
		...
		if doesFileExist(dirml.."/MZHelper/MainSetting.med") then
		print("{82E28C}×òåíèå íàñòðîåê...")
		local f = io.open(dirml.."/MZHelper/MainSetting.med")
			local setf = f:read("*a")
			f:close()
			...
			if res and type(set) == "table" then 
				...
				os.remove(dirml.."/MZHelper/MainSetting.med")
		...
		if not cb_imageDis.v then
			disSite = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/discord-site.png")
			disRole = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/discord-role.png")
			disMed = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/discord-med.png")
			disNick = imgui.CreateTextureFromFile(dirml.."/MZHelper/files/discord-nick.png")
		end

	print("{82E28C}×òåíèå íàñòðîåê êîìàíä...")
	if doesFileExist(dirml.."/MZHelper/cmdSetting.med") then
	--register cmd
		local f = io.open(dirml.."/MZHelper/cmdSetting.med")
		...
		if res and type(keys) == "table" then
			...
			os.remove(dirml.."/MZHelper/cmdSetting.med")
		end
	else
	...
	if doesFileExist(dirml.."/MZHelper/bindSetting.med") then
		local f = io.open(dirml.."/MZHelper/bindSetting.med")
		...
		if res and type(list) == "table" then
			...
			os.remove(dirml.."/MZHelper/bindSetting.med")
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("mz", function() mainWin.v = not mainWin.v end)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ñêðèïò èíèöèàëèçèðîâàí.", 0xff00ff)
		repeat wait(100) until sampIsLocalPlayerSpawned()
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
		myNick = sampGetPlayerNickname(myid)
		
		sampAddChatMessage(string.format("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðèâåòñòâóþ, %s. Äëÿ àêòèâàöèè ãëàâíîãî ìåíþ ïðîïèøèòå â ÷àò {22E9E3}/mz.", sampGetPlayerNickname(myid):gsub("_"," ")), 0xff00ff)
		wait(200)
		if buf_nick.v == "" then 
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïîõîæå ó òåáÿ íå íàñòðîåíà îñíîâíàÿ èíôîðìàöèÿ. ", 0xff00ff)
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Çàéäè â ãëàâíîì ìåíþ â ðàçäåë \"Íàñòðîéêè\" è íàñòðîé ñåáå âñ¸ ïî \"ôýí-øóþ\".", 0xff00ff)
		end
...
function mainSet()
	...
	imgui.PushItemWidth(300);
		...
		imgui.PushItemWidth(278);
			...
			--		imgui.OpenPopup(u8"MZ | Èçìåíåíèå íàçâàíèÿ áîëüíèöû")
			--	end
			--imgui.PopStyleVar(1)
			--imgui.SameLine(22)
			...
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##2", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"MZ | Èçìåíåíèå íàçâàíèÿ äîëæíîñòè")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
	...
	imgui.PopItemWidth()
	imgui.EndGroup()
	if imgui.BeginPopupModal(u8"MZ | Èçìåíåíèå íàçâàíèÿ áîëüíèöû", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	if imgui.BeginPopupModal(u8"MZ | Èçìåíåíèå íàçâàíèÿ äîëæíîñòè", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
...
function imgui.OnDrawFrame()
	if mainWin.v then
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " MZHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			imgui.BeginChild("settig", imgui.ImVec2(0, 390), true)
				...
				if imgui.CollapsingHeader(u8"Öåíîâàÿ ïîëèòèêà") then
					...
					imgui.BeginGroup()
						...
						imgui.PopItemWidth()
					imgui.EndGroup();
					imgui.TextWrapped(u8"Áîëåå ïîäðîáíî ìîæåòå óçíàòü íà ôîðóìå ïî ïóòè forum.rodina-rp.com -> Çàïàäíûé îêðóã -> Ãîñ. ñòóðêòóðû -> Ìèí.Çäðàâ.")
				end
				--imgui.Dummy(imgui.ImVec2(0, 3))
			...
			for i,v in ipairs(chgName.rank) do
				...
				local f = io.open(dirml.."/MZHelper/MainSetting.med", "w")
				f:write(encodeJson(setting))
				f:flush()
				f:close()
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàñòðîéêè ñîõðàíåíû.", 0xff00ff)
				needSave = false
			end
			imgui.PopStyleColor(1)
			imgui.EndGroup()
						...
						if #spur.list ~= 20 then
							for i = 1, 20 do
								if not table.concat(spur.list, "|"):find("Øïàðãàëêà '"..i.."'") then
									...
									spur.text.v = ""
									spurBig.v = false
									local f = io.open(dirml.."/MZHelper/Øïàðãàëêè/Øïàðãàëêà '"..i.."'.txt", "w")
									f:write("")
									f:flush()
									f:close()
									break
					...
					--	
						...
						--	imgui.SetCursorPosX(415)
							if imgui.Button(u8"Óäàëèòü", imgui.ImVec2(260, 20)) then
								if doesFileExist(dirml.."/MZHelper/Øïàðãàëêè/"..spur.list[spur.select_spur]..".txt") then
									os.remove(dirml.."/MZHelper/Øïàðãàëêè/"..spur.list[spur.select_spur]..".txt")
								end
							...
							if imgui.Button(u8"Ñîõðàíèòü", imgui.ImVec2(260, 20)) then
								...
								if spur.name.v ~= "" then 
										...
										if doesFileExist(dirml.."/MZHelper/Øïàðãàëêè/"..name..".txt") and spur.list[spur.select_spur] ~= name then
											bool = true
								...
								if not bool then
									local f = io.open(dirml.."/MZHelper/Øïàðãàëêè/"..name..".txt", "w")
						...
						elseif not spurBig.v and (spur.select_spur >= 1 and spur.select_spur <= 20) then
							...
							if imgui.Button(u8"Ðåäàêòèðîâàòü", imgui.ImVec2(170, 20)) then
								spur.edit = true
								local f = io.open(dirml.."/MZHelper/Øïàðãàëêè/"..spur.list[spur.select_spur]..".txt", "r")
				...
				imgui.EndChild();
					if cmdBind[selected_cmd].rank <= num_rank.v+1 then
						...
						if imgui.Button(u8"Íàçíà÷èòü êëàâèøó", imgui.ImVec2(140, 20)) then 
							imgui.OpenPopup(u8"MZ | Óñòàíîâêà êëàâèøè äëÿ àêòèâàöèè");
						...
						if imgui.Button(u8"Î÷èñòèòü àêòèâàöèþ", imgui.ImVec2(140, 20)) then 
							...
							cmdBind[selected_cmd].key = {}
								local f = io.open(dirml.."/MZHelper/cmdSetting.med", "w")
						...
						imgui.Separator()
							for i,v in ipairs(binder.list) do
								if imgui.Selectable(u8(binder.list[i].name), binder.select_bind == i) then 
									...
									if doesFileExist(dirml.."/MZHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
										local f = io.open(dirml.."/MZHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "r")
					...
					--	
						if binder.edit then
							...
							if imgui.Button(u8"Íàçíà÷èòü êëàâèøó", imgui.ImVec2(150, 20)) then 
								imgui.OpenPopup(u8"MH | Óñòàíîâêà êëàâèøè äëÿ àêòèâàöèè")
								editKey = true
							end 
							imgui.SameLine()
						...
						--	imgui.SetCursorPosX(345)
							if imgui.Button(u8"Óäàëèòü", imgui.ImVec2(127, 20)) then
								...
								binder.key = {}
								if doesFileExist(dirml.."/MZHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt") then
									os.remove(dirml.."/MZHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt")
								end
								table.remove(binder.list, binder.select_bind) 
								local f = io.open(dirml.."/MZHelper/bindSetting.med", "w")
							...
							if imgui.Button(u8"Ñîõðàíèòü", imgui.ImVec2(127, 20)) then
								...
								if not bool then
									...
									local f = io.open(dirml.."/MZHelper/bindSetting.med", "w")
									local ftx = io.open(dirml.."/MZHelper/Binder/bind-"..binder.list[binder.select_bind].name..".txt", "w")
									f:write(saveJS)
									ftx:write(text)
									f:flush()
									ftx:flush()
			...
			if select_menu[6] then
				...
				imgui.BeginChild("help but", imgui.ImVec2(0,0), true)
					...
					if imgui.IsItemClicked(0) then
						print(shell32.ShellExecuteA(nil, 'open', dirml.."/MZHelper/Øïàðãàëêè/", nil, nil, 1))
					end
					--
					imgui.Bullet(); imgui.SameLine()
					...
					imgui.TextColoredRGB("{FF5F29}/mh-delete {FFFFFF} - óäàëèòü ñêðèïò")
					--
					imgui.Bullet(); imgui.SameLine()
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Text(u8"MZHelper")
					imgui.Spacing()
					imgui.TextWrapped(u8"\tÑêðèïò áûë ðàçðàáîòàí äëÿ ïðîåêòà Rodina Role Play ñ ïîääåðæêîé ðàáîòû íà ñåðâåðå Çàïàäíûé Îêðóã äëÿ îáëåã÷åíèÿ ðàáîòû ñîòðóäíèêàì áîëüíèö. Áëàãîäàðÿ ýòîìó ïðèëîæåíèþ Âû ïîëó÷èòå ïîëíûé êîìïëåêñ àâòîìàòèçàöèè ìíîãèõ äåéñòâèé è íàñëàæäåíèå îò ïîëüçîâàíèÿ.\nÎáíîâëåíèÿ âûõîäÿò ïî ìåðå äîáàâëåíèÿ íîâîââåäåíèé è èñïðàâëåíèé îøèáîê.")
					imgui.Dummy(imgui.ImVec2(0, 10))
					imgui.Bullet()
					imgui.TextColoredRGB("Ðàçðàáîò÷èê - {FFB700}Äìèòðèé Öîé è Äæåéìñ Õàððèñîí")
					imgui.Bullet()
					...
					--imgui.TextColoredRGB("Áëàãîäàðíîñòü blast.hk çà ìåòîäè÷åñêóþ èíôîðìàöèþ è åãî ïîëüçîâàòåëÿì â ðåøåíèè âîïðîñîâ.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Íàøëè áàã èëè îøèáêó, èëè æå õîòèòå âèäåòü ÷òî-òî íîâîå, íàïèøè íàì"); imgui.SameLine(); imgui.Text(fa.ICON_ARROW_DOWN)
						imgui.SetCursorPosX(20)
						imgui.Text(fa.ICON_LINK)
						imgui.SameLine()
						imgui.TextColoredRGB("Äëÿ ñâÿçè: VK: {74BAF4}https://vk.com/frostrylit") --èëè Discord - {74BAF4}TheVitek#2160")
							if imgui.IsItemHovered() then imgui.SetTooltip(u8"Êëèêíèòå ËÊÌ, ÷òîáû ñêîïèðîâàòü, èëè ÏÊÌ, ÷òîáû îòêðûòü â áðàóçåðå")  end
							if imgui.IsItemClicked(0) then setClipboardText("https://vk.com/frostrylit") end
							if imgui.IsItemClicked(1) then print(shell32.ShellExecuteA(nil, 'open', 'https://vk.com/frostrylit', nil, nil, 1)) end
							imgui.SameLine()
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Æåëàþùèå ïîääåðæàòü ðàçðàáîòêó äåíþøêîé - ".."{68E15D}\"Îòêðûòü\"")
							if imgui.IsItemHovered() then imgui.SetTooltip(u8"Êëèêíèòå, ÷òîáû îòêðûòü ññûëêó")  end
							if imgui.IsItemClicked(0) then print(shell32.ShellExecuteA(nil, 'open', 'qiwi.ru', nil, nil, 1)) end
						...
						if imgui.Button(u8"Óäàëèòü ñêðèïò", imgui.ImVec2(160, 20)) then 
							...
							sampAddChatMessage("", 0xff00ff)
							sampAddChatMessage("", 0xff00ff)
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Âíèìàíèå! Ïîäòâåðäèòå óäàëåíèå êîìàíäîé {77DF63}/mh-delete.", 0xff00ff)
							mainWin.v = false
						--	sampShowDialog(1002, "{E94C4C}MZHelper | {8EE162}Óäàëåíèå", remove, "Ïðèíÿë", "")
						end
				imgui.EndChild()
			...
			imgui.BeginChild("discord", imgui.ImVec2(0, 0), false)
				...
				imgui.TextWrapped(u8"Íàæìèòå 'Download for Windows', åñëè õîòèòå èñïîëüçîâàòü ïðîãðàììó íà êîìïüþòåðå, èëè 'Îòêðûòü â áðàóçåðå', åñëè õîòèòå ïîëüçîâàòüñÿ Äèñêîðäîì ïðÿìî â áðàóçåðå, íî ó÷òèòå, ÷òî â áðàóçåðå Âû íå ñìîæåòå ãîâîðèòü ïî àêòèâàöèè íà êëàâèøó.")
				if disSite then
				...
				imgui.TextColoredRGB("\t1. Íàæìèòå íà òåêñòîâûé êàíàë, óêàçàííûé ëåâîé êðàñíîé ñòðåëêîé. \n\t2. Ïðîïèøèòå êîìàíäó !role. Ïîñëå ââîäà êîìàíäû äîëæíî ïîëó÷èòüñÿ òàê(Ïîêàçàíî ïðàâîé ñòðåëêîé):")
				if disRole then
					imgui.SetCursorPosX(25)
					imgui.Image(disRole, imgui.ImVec2(586, 340))
				end
				imgui.TextColoredRGB("\t4. Ïîïðîñèòå â èãðå ñîòðóäíèêà îò 9 ðàíãà è âûøå, ÷òîáû âûäàëè ðîëü Âàì, íàïèñàâ, ÷òî Âû\n çàïðîñèëè óæå.")
				...
				imgui.TextColoredRGB("\t5. Èçìåíèòå ñåáå íèêíåéì. Äëÿ ýòîãî â êàíàëå êëèêíèòå ïî ñåáå ÏÊÌ (ïðàêâîé êíîïêîé ìûøè). \nÏîñëå ÷åãî âûáåðåòå ïóíêò {F2D045}'Èçìåíèòü íèêíåéì'. {FFFFFF}Íèê äåëàåì ïî ôîðìå, ê ïðèìåðó Âû ñîòðóäíèê \náîëüíèöû ËÂ, äåëàåì íèê: {B3F637}[LVMC][Âàø ðàíã] Íèê Íåéì")
				if disNick then
					imgui.SetCursorPosX(25)
			...
			--///Óñòàíîâêà êëàâèøè
				...
				if imgui.BeginPopupModal(u8"MZ | Óñòàíîâêà êëàâèøè äëÿ àêòèâàöèè", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	--	imgui.SetWindowFontScale(1.1)
		if spur.edit then
				...
				if imgui.Button(u8"Óäàëèòü", imgui.ImVec2(357, 20)) then
					...
					table.remove(spur.list, spur.select_spur) 
					spur.select_spur = -1
					if doesFileExist(dirml.."/MZHelper/Øïàðãàëêè/"..u8:decode(spur.select_spur)..".txt") then
						os.remove(dirml.."/MZHelper/Øïàðãàëêè/"..u8:decode(spur.select_spur)..".txt")
					end
					spur.name.v = ""
					spurBig.v = false
					spur.edit = false
			...
			imgui.BeginChild("spur spec", imgui.ImVec2(1081, 730), true)
				if doesFileExist(dirml.."/MZHelper/Øïàðãàëêè/"..spur.list[spur.select_spur]..".txt") then
					for line in io.lines(dirml.."/MZHelper/Øïàðãàëêè/"..spur.list[spur.select_spur]..".txt") do
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8"Íà÷àòü", imgui.ImVec2(60, 20)) then
					if sobes.selID.v ~= "" then
						...
						sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Óêàæèòå id èãðîêà äëÿ íà÷àëà ñîáåñåäîâàíèÿ.", 0xff00ff)
					end
				end
				...
				imgui.EndChild()
					...
					elseif dep.bool[7] then
						...
						imgui.Spacing()
							...
							if imgui.IsItemClicked(0) then
								print(shell32.ShellExecuteA(nil, 'open', dirml.."/MZHelper/Äåïàðòàìåíò/", nil, nil, 1))
							end
						imgui.Spacing()
						imgui.Spacing()
						if imgui.Button(u8"Ïîäàòü", imgui.ImVec2(225, 30)) then
							...
							-- 	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Gov Íîâîñòè íàçíà÷èòü ìîæíî òîëüêî îò 0 äî 15 ìèíóò.", 0xff00ff)
							-- 	return
							-- end
							
								if doesFileExist(dirml.."/MZHelper/Äåïàðòàìåíò/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") then
								--	print(u8:decode(dep.news[dep.newsN.v+1]))
									for line in io.lines(dirml.."/MZHelper/Äåïàðòàìåíò/"..u8:decode(dep.news[dep.newsN.v+1])..".txt") do
										sampSendChat(line)
										wait(1000)
									end
								end
			...
			if #updinfo < 5 then
				...
				if newversion == scr.version then
					...
					imgui.BeginChild("update log", imgui.ImVec2(0, 0), true)
						if doesFileExist(dirml.."/MZHelper/files/update.txt") then
							for line in io.lines(dirml.."/MZHelper/files/update.txt") do
...
function onHotKeyCMD(id, keys)
	if thread:status() == "dead" then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Â äàííûé ìîìåíò ïðîèãðûâàåòñÿ îòûãðîâêà.", 0xff00ff)
	end
end
...
local function playBind(tb)
	if not tb.debug.file or #tb.debug.close > 0 then
		if not tb.debug.file then
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ôàéë ñ òåêñòîì áèíäà íå îáíàðóæåí. ", 0xff00ff)
		elseif #tb.debug.close > 0 then
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Äèàëîã, íà÷àëî êîòîðîãî ÿâëÿåòñÿ ñòðîêà ¹"..tb.debug.close[#tb.debug.close]..", íå çàêðûò òåãîì {dialogEnd}", 0xff00ff)
		end
		addOneOffSound(0, 0, 0, 1058)
		return false
	end
		...
		for k, v in pairs(binder.list) do
			if sKeys == tostring(table.concat(v.key, " ")) then
				thread = lua_thread.create(function()		
					local dir = dirml.."/MZHelper/Binder/bind-"..v.name..".txt"	
					local tb = {}
...
function getSpurFile()
	spur.list = {}
    local search, name = findFirstFile("moonloader/MZHelper/Øïàðãàëêè/*.txt")
	while search do
...
lua_thread.create(function()
	if doesDirectoryExist(dirml.."/MZHelper/Äåïàðòàìåíò/") then
		if doesFileExist(dirml.."/MZHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé.txt") or not doesFileExist(dirml.."/MedicalHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé ËÑÌÖ.txt") then
			os.remove(dirml.."/MZHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé.txt")
			local f = io.open(dirml.."/MZHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé ËÑÌÖ.txt", "w")
			f:write(govls)
			f:flush()
			f:close()
			local f = io.open(dirml.."/MZHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé ÑÔÌÖ.txt", "w")
			f:write(govsf)
			f:flush()
			f:close()
			local f = io.open(dirml.."/MZHelper/Äåïàðòàìåíò/Äåíü îòêðûòûõ äâåðåé ËÂÌÖ.txt", "w")
			f:write(govlv)
			f:flush()
			f:close()
		end
		dep.news = {}
		local search, name = findFirstFile("moonloader/MZHelper/Äåïàðòàìåíò/*.txt")
		while search do
			if not name then findClose(search) else
...
function tags(par)
		...
		if par:find("{getNickByID:%d+}") then
			for v in par:gmatch("{getNickByID:%d+}") do
				...
				if sampIsPlayerConnected(id) then
					...
					sampAddChatMessage("{FFFFFF}[{ff00ff}MZ:Îøèáêà{FFFFFF}]: Ïàðàìåòð {getNickByID:ID} íå ñìîã âåðíóòü íèê èãðîêà. Âîçìîæíî èãðîê íå â ñåòè.", 0xff00ff)
					par = par:gsub(v,"")
				end
			end
		end
		...
		if par:find("{getNickByTarget}") then
			if targID ~= nil and targID >= 0 and targID <= 1000 and sampIsPlayerConnected(targID) then
				...
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZ:Îøèáêà{FFFFFF}]: Ïàðàìåòð {getNickByTarget} íå ñìîã âåðíóòü íèê èãðîêà. Âîçìîæíî Âû íå öåëèëèñü íà èãðîêà, ëèáî îí íå â ñåòè.", 0xff00ff)
...
funCMD = {} 
function funCMD.del()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Âû óñïåøíî óäàëèëè ñêðèïò.", 0xff00ff)
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Âûãðóçêà ñêðèïòà èç èãðû...", 0xff00ff)
	os.remove(scr.path)
	scr:reload()
end
function funCMD.lec(id)
	...
	if not u8:decode(buf_nick.v):find("[à-ÿÀ-ß]+%s[à-ÿÀ-ß]+") then
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïîäîæäèòå-êà, ñíà÷àëà íóæíî çàïîëíèòü áàçîâóþ èíôîðìàöèþ. {90E04E}/mz > Íàñòðîéêè > Îñíîâíàÿ èíôîðìàöèÿ", 0xff00ff)
		return
	end
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				sampSendChat(string.format("Çäðàâñòâóéòå, ÿ %s, äåæóðíûé ëå÷àùèé âðà÷.", u8:decode(buf_nick.v)))
				wait(2000)
				sampSendChat("Íà ÷òî æàëóåòåñü?")
				wait(1000)
					sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà  {23E64A}Enter{FFFFFF} äëÿ ïðîäîëæåíèÿ.", 0xff00ff)
					addOneOffSound(0, 0, 0, 1058)
					local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Ïðîäîëæèòü")
					while true do
						wait(0)
				...
				sampSendChat("/do Ìåäèöèíñêàÿ ñóìêà íà ëåâîì ïëå÷å")
				wait(2000)
				sampSendChat(chsex("/me îòêðûâ ñóìêó, äîñòàë èç íå¸ ïà÷êó òàáëåòîê", "/me îòêðûâ ñóìêó, äîñòàëà èç íå¸ ïà÷êó òàáëåòîê"))
				wait(2000)
				sampSendChat(chsex("/me âûäàâèë òàáëåòêó èç ïà÷êè", "/me âûäàâèëà òàáëåòêó èç ïà÷êè"))
				wait(2000)
				sampSendChat(chsex("/me ïåðåäàë òàáëåòêó áîëüíîìó", "/me ïåðåäàëà òàáëåòêó áîëüíîìó"))
				wait(2000)
				sampSendChat("/do Ëåêàðñòâî â ëåâîé ðóêå.")
				wait(2000)
				sampSendChat("Õîðîøåãî äíÿ, âûçäîðàâëèâàéòå")
				wait(100)
	...
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /hl [id èãðîêà].", 0xff00ff)
	end
end
function funCMD.med(id)
	...
	thread = lua_thread.create(function()
		--[[
			...
			sampSendChat("Ïðåäîñòàâüòå ïîæàëóéñòà Âàø ïàñïîðò äëÿ îïðåäåëåíèÿ òðåáîâàíèé.")		
				...
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà öèôðó âåðõíåé ïàíåëè äëÿ âûáîðà âèäà ìåä.óñëóãè.", 0xff00ff)
				addOneOffSound(0, 0, 0, 1058)
				while true do
					wait(0)
			...
			while true do
				...
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà öèôðó âåðõíåé ïàíåëè ïîñëå îòâåòà ïàöèåíòà.", 0xff00ff)
				addOneOffSound(0, 0, 0, 1058)
				local diag = 0
				local time = 0
				while true do
		...
		local dir = dirml.."/MZHelper/rp-medcard.txt"	
		local tb = {}
		tb = strBinderTable(dir)
		tb.sleep = 1.85
		tb.vars["playerID"] = id
	...
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó: /mc [id èãðîêà].", 0xff00ff)
	end
...
function funCMD.narko(id)
	if thread:status() ~= "dead" then 
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /narko [id èãðîêà].", 0xff00ff)
		end
end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(500)
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà öèôðó âåðõíåé öèôðîâîé ïàíåëè ðàâíàÿ êîëè÷åñòâó âûäàâàåìûõ ðåöåïòîâ.", 0xff00ff)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /rec [id èãðîêà].", 0xff00ff)
		end
end
...
--		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: ×òîáû çàñòóïèòü íà ìîáèëüíûé ïîñò, Âàì íåîáõîäèìî ñíà÷àëà ñåñòü â êàðåòó.", 0xff00ff)
--		addOneOffSound(0, 0, 0, 1058)
--	else
...
--			else
--				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Óêàæèòå îáñòàíîâêó, íàïðèìåð, /post Ñïîêîéíî.", 0xff00ff)
--			end
--		end
--	end
--end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(1000)
					addOneOffSound(0, 0, 0, 1058)
					sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïàñïîðò èãðîêà àâòîìàòè÷åñêè çàêðîåòñÿ, ïîñëå ÷åãî ïðîäîëæèòñÿ äèàëîã..", 0xff00ff)
					repeat wait(0) until sampIsDialogActive()
					while sampIsDialogActive() do
						local memory = require 'memory'
						memory.setint64(sampGetDialogInfoPtr()+40, bool and 1 or 0, true)
				...
				sampSendChat("/b Ïîêàæèòå òàòóèðîâêè ñ ïîìîùüþ êîìàíäû /showtatu")
					sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà  {23E64A}Enter{FFFFFF} äëÿ ïðîäîëæåíèÿ èëè {23E64A}Page Down{FFFFFF}, ÷òîáû çàêîí÷èòü äèàëîã.", 0xff00ff)
					addOneOffSound(0, 0, 0, 1058)
					local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Ïðîäîëæèòü")
					while true do
					wait(0)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /tatu [id èãðîêà].", 0xff00ff)
		end	
end
function funCMD.warn(text)
	...
	if num_rank.v+1 < 8 then
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Äàííàÿ êîìàíäà Âàì íåäîñòóïíà. Ïîìåíÿéòå äîëæíîñòü â íàñòðîéêàõ ñêðèïòà, åñëè ýòî òðåáóåòñÿ.", 0xff00ff)
		return
	end
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /+warn [id èãðîêà] [ïðè÷èíà].", 0xff00ff)
		end
end
function funCMD.uwarn(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /-warn [id èãðîêà].", 0xff00ff)
		end
end
function funCMD.inv(id)
	...
	if num_rank.v+1 < 9 then
		...
		if id:find("(%d+)") then
		thread = lua_thread.create(function()
					sampSendChat("/do Â êàðìàíå õàëàòà íàõîäÿòñÿ êëþ÷è îò øêàô÷èêîâ.")
					wait(2000)
					sampSendChat("/me ïîòÿíóâøèñü âî âíóòðåííèé êàðìàí õàëàòà, "..chsex("äîñòàë","äîñòàëà").." îòòóäà êëþ÷.")
					wait(2000)
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /inv [id èãðîêà].", 0xff00ff)
		end
end
function funCMD.unv(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /unv [id èãðîêà] [ïðè÷èíà].", 0xff00ff)
		end
end
function funCMD.mute(text)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /+mute [id èãðîêà] [âðåìÿ â ìèíóòàõ] [ïðè÷èíà].", 0xff00ff)
		end
end
function funCMD.umute(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /-mute [id èãðîêà].", 0xff00ff)
		end
end
function funCMD.rank(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /gr [id èãðîêà] [íîìåð ðàíãà].", 0xff00ff)
		end
end
function funCMD.osm()
	if thread:status() ~= "dead" then 
		...
		thread = lua_thread.create(function()
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íàæìèòå íà {23E64A}Enter{FFFFFF}, åñëè ãîòîâû íà÷àòü îñìîòð.", 0xff00ff)
				addOneOffSound(0, 0, 0, 1058)
...
function funCMD.expel(par)
	...
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Èñïîëüçóéòå êîìàíäó /exp [id èãðîêà] [Ïðè÷èíà].", 0xff00ff)
	end
end
		
function funCMD.openupd()
	print(shell32.ShellExecuteA(nil, 'open', "http://forum.arizona-rp.com/index.php?threads/Ìèíèñòåðñòâî-Çäðàâîîõðàíåíèÿ-Îôèöèàëüíûé-ñêðèïò-äëÿ-áîëüíèö-medical-helper.1119179/", nil, nil, 1))
end

function funCMD.update()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðîèçâîäèòñÿ ñêà÷èâàíèå íîâîé âåðñèè ñêðèïòà...", 0xff00ff)
	local dir = dirml.."/MZHelper.lua"
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðîèçîøëà îøèáêà ïðè ñêà÷èâàíèè îáíîâëåíèÿ. Àêòèâàöèÿ ðåçåðâîãî èñòî÷íèêà...", 0xff00ff)
		...
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ñêà÷èâàíèå çàâåðøåíî, ïåðåçàãðóçêà áèáëèîòåê...", 0xff00ff)
			reloadScripts()
			showCursor(false)
		end
	end)
...
Ëèáî ñêà÷àéòå ôàéë ñêðèïòà îòäåëüíî. Ññûëêà äëÿ ñêà÷èâàíèÿ óæå ñêîïèðîâàíà.
	...
	3. Óäàëèòå ôàéë MZHelper.lua
	4. Ïåðåìåñòèòå ñêà÷åííûé ôàéë â ïàïêó Moonloader. 
	{FCB32B}5. Óáåäèòåñü, ÷òî íàçâàíèå íå ñîäåðæèò ëèøíèõ ñèìâîëîâ, íàïðèìåð MZHelper{F65050}(1){FCB32B}.luac
]]
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðîèçâîäèòñÿ ñêà÷èâàíèå íîâîé âåðñèè ñêðèïòà...", 0xff00ff)
	local dir = dirml.."/MZHelper.lua"
	local url = urlupd
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				print("{FF0000}Îøèáêà ïðè ïîïûòêå ñêà÷àòü ôàéë.") 
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðîèçîøëà îøèáêà ïðè ñêà÷èâàíèè îáíîâëåíèÿ. Ïîõîæå, ñêà÷èâàíèþ ÷òî-òî ìåøàåò.", 0xff00ff)
...
function funCMD.updateCheck()
	sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Ïðîâåðÿåì íàëè÷èå îáíîâëåíèé...", 0xff00ff)
		local dir = dirml.."/MZHelper/files/update.med"
		local url = "https://raw.githubusercontent.com/TheMrThor/MedicalHelper/master/update.med"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
				wait(1000)
				if doesFileExist(dirml.."/MZHelper/files/update.med") then
					local f = io.open(dirml.."/MZHelper/files/update.med", "r")
					...
					urlupd = upd.url
						if upd.version == scr.version then
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Âñ¸ îòëè÷íî, Âû èñïîëüçóåòå ñàìóþ íîâóþ âåðñèþ ñêðèïòà.", 0xff00ff)
						else
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: {4EEB40}Èìååòñÿ îáíîâëåíèå.{FFFFFF} Íàïèøè {22E9E3}/update{FFFFFF} äëÿ ïîëó÷åíèÿ èíôîðìàöèè.", 0xff00ff)
							wait(5000)
							updWin.v = true
						end
					end
		...
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
					wait(1000)
					if doesFileExist(dirml.."/MZHelper/files/update.txt") then
					local f = io.open(dirml.."/MZHelper/files/update.txt", "r")
					updinfo = f:read("*a")
					f:close()
					end
				end)
...
--			sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: Íà êàðòå áûëà âûñòàâëåíà ìåòêà ìåñòà íàçíà÷åíèÿ.", 0xff00ff)
--			addOneOffSound(0, 0, 0, 1058)
--		elseif but == 0 then
--		end
--	end
