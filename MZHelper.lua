...
lua_thread.create(function()
	if doesDirectoryExist(dirml.."/MZHelper/Департамент/") then
		if doesFileExist(dirml.."/MZHelper/Департамент/День открытых дверей.txt") or not doesFileExist(dirml.."/MedicalHelper/Департамент/День открытых дверей ЛСМЦ.txt") then
			os.remove(dirml.."/MZHelper/Департамент/День открытых дверей.txt")
			local f = io.open(dirml.."/MZHelper/Департамент/День открытых дверей ЛСМЦ.txt", "w")
			f:write(govls)
			f:flush()
			f:close()
			local f = io.open(dirml.."/MZHelper/Департамент/День открытых дверей СФМЦ.txt", "w")
			f:write(govsf)
			f:flush()
			f:close()
	...
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				...
				wait(2000)
				sampSendChat(chsex("/me открыв сумку, достал из неё пачку таблеток", "/me открыв сумку, достала из неё пачку таблеток"))
				wait(2000)
				...
				if doesFileExist(dirml.."/MZHelper/files/update.med") then
					...
					urlupd = upd.url
						if upd.version == scr.version then
							...
							sampAddChatMessage("{FFFFFF}[{ff00ff}MZHelper{FFFFFF}]: {4EEB40}Имеется обновление.{FFFFFF} Напиши {22E9E3}/update{FFFFFF} для получения информации.", 0xff00ff)
							wait(5000)
							updWin.v = true
						end
					end
